...
<div class="anthill-scene">
    <div class="anthill-title">
        ...
        <p>Mraveniště je tvoje základna. Nakupuj komůrky, stav místnosti a plň jejich vnitřní úkoly.</p>
        @if($readonly ?? false)
    ...
    @foreach($slots as $slot)
        ...
        <div class="room slot"
            ...
            style="left:{{ $slot->layout_x }}%; top:{{ $slot->layout_y }}%;">
            @if($placedBuilding)
                @if($readonly ?? false)
                    <img src="{{ $placedBuilding->svg_asset_path }}" alt="{{ $placedBuilding->name }}"><b>{{ $placedBuilding->name }}</b>
                @else
                <a href="/budovy/{{ $placedBuilding->slug }}"><img src="{{ $placedBuilding->svg_asset_path }}" alt="{{ $placedBuilding->name }}"><b>{{ $placedBuilding->name }}</b></a>
                @endif
            @elseif($isOwned)
                <img src="/assets/placeholders/slot-empty.svg" alt="Prázdný slot">
            @else
                ...
                <div class="modal-window">
                    ...
                    @if($isOwned)
                        ...
                        @if(count($ownedBuildingIds) < $buildings->count())
                            <form method="post" action="/mraveniste/build">@csrf
                    ...
                    @else
                        ...
                        <form method="post" action="/mraveniste/slots/{{ $slot->id }}/buy">@csrf
