...
@php
    ...
    $contentMode = request('mode', 'palouk');
@endphp
<h1>Úprava hry</h1>
...
<div class="panel" style="margin-bottom:16px">
    <div class="building-tab-switch" style="margin:0">
        <a class="btn {{ $contentMode === 'palouk' ? 'primary' : '' }}" href="/admin/obsah?mode=palouk">Palouk</a>
        <a class="btn {{ $contentMode === 'mraveniste' ? 'primary' : '' }}" href="/admin/obsah?mode=mraveniste">Mraveniště</a>
    </div>
</div>
...
@if($contentMode === 'mraveniste')
    ...
    @foreach($buildings as $building)
        ...
        <details class="card" style="margin-bottom:12px" open>
            ...
            <h3>Podúkoly</h3>
            @forelse($tasksForBuilding as $task)
                <details class="card" style="margin:10px 0">
                    <summary><b>{{ $task->title }}</b></summary>
                    <form method="post" action="/admin/obsah/budovy/ukoly/{{ $task->id }}" enctype="multipart/form-data">
                        ...
                        <label>Co se splněním odemkne</label>
                        <input name="unlock_description" value="{{ $task->unlock_description ?? '' }}" maxlength="240">
            ...
            @empty
                <p class="muted">Tato budova zatím nemá žádné podúkoly.</p>
            @endforelse
        </details>
    @endforeach
@else
    ...
    @foreach($badges as $badge)
        <details class="card" style="margin-bottom:10px">
            ...
            <form method="post" action="/admin/odznacky/{{ $badge->id }}" enctype="multipart/form-data">
                ...
                <div class="grid">
                    <div><label>Nahrát novou ikonu</label><input name="icon" type="file" accept="image/png,image/jpeg,image/webp"></div>
                    <div><label>Vybrat uloženou ikonu</label><select name="existing_icon_path"><option value="">Ponechat aktuální</option>@foreach($imageFiles as $file)<option value="{{ $file->public_path }}">{{ $file->original_name }} - {{ $file->public_path }}</option>@endforeach</select></div>
                </div>
